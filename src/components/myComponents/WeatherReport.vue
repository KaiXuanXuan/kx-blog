<template>
  <div class="space-y-2">
    <Card v-if="getDataSuccess" class="items-center justify-center gap-1 bg-linear-to-br from-cyan-50 to-purple-50">
      <div class="p-2">
    ...
    <Collapsible v-model:open="isOpen" class="w-full space-y-2">
      <CollapsibleTrigger class="w-full">
        <Card v-if="getDataSuccess" class="p-1 rounded-md cursor-pointer bg-blue-300 text-white">3日天气</Card>
        <Card v-else @click="getWeather" class="p-1 rounded-md cursor-pointer bg-blue-300 text-white">{{ isLoading == true ? '获取中..' : '获取天气' }}</Card>
      </CollapsibleTrigger>
